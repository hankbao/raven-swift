    ...
    */
        ...
        exception:[String : String]) -> [String: AnyObject]
    {
        ...
        let returnDict : [String: AnyObject] = ["event_id" : self.generateUUID() as AnyObject,
            "project"   : (self.config?.projectId ?? "") as AnyObject,
            "timestamp" : self.dateFormatter.string(from: Date()) as AnyObject,
            "level"     : level.rawValue as AnyObject,
            "platform"  : "swift" as AnyObject,
            "extra"     : extra as AnyObject,
            "tags"      : tags as AnyObject,
            "logger"    : (self.logger ?? "") as AnyObject,
            "message"   : message as AnyObject,
            "culprit"   : (culprit ?? "") as AnyObject,
            "stacktrace": (stacktraceDict) as AnyObject,
            "exception" : exception as AnyObject,
            "user"      : (user as AnyObject?) ?? "" as AnyObject ]
    ...
    fileprivate func sendJSON(_ JSON: Data?) {
        ...
        let task = session.dataTask(with: request) {
            (_, response, error) in
            if let error = error {
                let errorKey: String? = (error as NSError).userInfo[NSURLErrorFailingURLStringErrorKey] as? String
